...
\chead{}
\rhead{\bf Prot. Nr.~{{STRENGTHTESTID}} ; \today} % Bandymų protokolo numerį reikia įrašyti. 
...
\begin{center}
	{\Large Betono kubelinio stiprio nustatymas pagal {\bf LST EN 12390-3}\\
		Bandymų protokolas {\bf \Large \textcolor{blue}{Nr.~{{STRENGTHTESTID2}},  \today }}\\ % Bandymų protokolo numerį reikia įrašyti. Jis gali būti generuojamas  automatiškai programos, tačiau tas protokolo numeris negali kartotis, kiekvieno protokolo numeris turi būti vienintelis, unikalus
	}
\end{center}

{\hspace{2.5cm} {\bf Užsakovas:} \hfill \parbox{12.0cm}{{{STRENGTHTESTCLIENTCOMPANYNAME}}, {{STRENGTHTESTCLIENTCOMPANYADDRESS}}, Įmonės kodas, {{STRENGTHTESTCLIENTCOMPANYCODE}}}} % Reikia čia įrašyti užsakovo rekvizitus  iš programos atitinkamo įvedimo lango

\vspace{0.5cm}

{\hspace{2.5cm} {\bf Vykdytojas:} \hfill \parbox{12.0cm}{{{STRENGTHTESTEMPLOYEECOMPANYNAME}}, {{STRENGTHTESTEMPLOYEECOMPANYADDRESS}}, Įmonės kodas, {{STRENGTHTESTEMPLOYEECOMPANYCODE}}}} % Šitą lauką galima užharkodinti 
...
\hspace{5.25cm} {\bf Bandymai atlikti:} {{{TESTEXECUTIONDATE}}} % Reikia čia įrašyti bandymų atlikimo datą iš programos įvedimo lango

\hspace{5.25cm} \nin {\bf Bandiniai gauti:} {{{TESTSAMPLESRECEIVEDDATE}}} % Reikia čia įrašyti bandinių gavimo datą iš programos įvedimo lango

\vspace{0.5cm}
\hspace{5.25cm} {\bf Bandinius pristatė:} {{{TESTSAMPLESDELIVEREDBY}}}. % Reikia čia įrašyti kas bandinius pristatė, galima padaryti ir pasirinkimus: Užsakovas / Užsakovo įgaliotas atstovas / Vykdytojas  Į

\hspace{5.25cm} {\bf Bandė:} {{{STRENGTHTESTEXECUTEDBYNAME}}}. % Reikia čia įrašyti laboranto vardą ir pavardę

\hspace{5.25cm} {\bf Protokolą sudarė:} {{{STRENGTHTESTPROTOCOLCREATEDBYNAME}}}.% Reikia čia įrašyti protokolo sudarytojo vardą ir pavardę
...
{\bf Betono tipas pagal tankį}: {{{CONCRETETYPE}}}.% Čia reikia įrašyti betono tipą pagal tankį. dar gali būti kitas pasirinkimas - lengvasis betonas. T.y. iš viso du pasirinkimai: {sunkusis arba normalusi} ir {lengvasis}.
...
{\bf Gautos arba paimtos imties dydis:} $ {{TESTSAMPLESRECEIVEDCOUNT}} $ betoniniai kubeliai. 
	...
	{\bf Bendras bandinių išvaizdos apibūdinimas bandinių priėmimo metu:} {{TESTSAMPLESRECEIVEDCOMMENT}} % Šitą tekstą paimti iš programos lauko kuriame jis įdedamas pagal nutylėjimą (t.y. toks kaip tai yra parašyta dabar). Suprantama, kad naudotojas gaali pakeisti tekstą į reikiamą.
...
$ {{REJECTEDSAMPLECOUNT}} $ kubeliai (kubeliai) neatiktiniai. % Šis įrašas daromas jei yra išbrokuotų kubelių,
% Čia taip pat reikia įrašyti išbrokuotų kubelių skaičių

Bandymo rezultatai surasyti į 1 lentelę. %~\ref{table:bandymu_rezultatai}
...
\begin{table}[H] % Čia turėtu būti parametras [t] lentelės patalpinimui puslapio viršuje
	...
	\begin{tabular}
		...
		\hline
		3 & {{CROSSDATA31}} \\ {{CROSSDATA32}} \\ {{CROSSDATA33}} \\ {{CROSSDATA34}} \\ {{CROSSDATA35}} \\ {{CROSSDATA36}} & {{CROSSDATA37}} \\ {{CROSSDATA38}} \\ {{CROSSDATA39}} \\ {{CROSSDATA310}} \\ {{CROSSDATA311}} \\ {{CROSSDATA312}} & {{DESTRUCTIVEPOWER3}}  & {{CRUSHINGSTRENGTH3}} & \small{{{DATACOMMENT3}}} \\
		\hline
		\end{tabular}%
...
\begin{enumerate}
	\item \hspace{\parindent}Vidutinio gniuždomojo striprio įvertis $ f_{cm,cube} =  {{AVERAGECRUSHFORCE}} $ MPa % Čia reikia įrašyti apskaičiuotą betono vidutinio stiprio įvertį
	...
	\item  Charakteristinio stiprio įvertis $ f_{ck,cube} = {{CHARACTERISTICSTRENGTH}} $ MPa % Čia reikia įrašyti apskaičiuotą betono kubelinio stiprumo standartinio nuokrypio įvertį
